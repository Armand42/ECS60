  ...
  int **RAM2, int *compulsory)
{
  ...
  inf.read((char*) numInstructions, sizeof(int));   // 4000000
  inf.read((char*) numProcesses, sizeof(unsigned short)); // 10
  ...
  *RAM2 = buffer2 = new int[size];
  ...
  {
    inf.read(&process, sizeof(char));     // 3 - random
    inf.read((char*) &process_instructions, sizeof(unsigned));  //102000    // repeats until it adds up to 4000000
    for(unsigned i = 0; i < process_instructions; i++)
    {
      inf.read((char*) &address, sizeof(unsigned short));
      instructions[count].addr1 = startingAddresses[(int)process] + address * 4;    // multiplys for the number of bytes
      inf.read((char*) &address, sizeof(unsigned short));
...
{
  Instruction *instructions, instruction;     //carrying out the instructions of the file we created
  ...
  instructions =  readFile(argv[1], startingAddresses, &numProcesses,
